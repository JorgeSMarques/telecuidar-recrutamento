    ...
    if (res.statusCode === 200) {
      ...
      if (content) {
        ...
        if (
          parsed.nome &&
          parsed.nome.trim() !== '' &&
          currentName.indexOf('Candidato Temporário') !== -1
        ) {
        ...
        if (
          parsed.email &&
          parsed.email.trim() !== '' &&
          currentEmail.indexOf('@exemplo.com') !== -1
        ) {
        ...
        if (updated) {
          ...
          $app.logger().info('Candidato atualizado pela IA no create', 'id', e.record.id)
        }
      }
    } else {
